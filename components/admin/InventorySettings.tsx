...
export default function InventorySettings({
  ...
  standOnly: InventoryProduct
  withCamera: InventoryProduct
}) {
  ...
  async function saveInventory(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch('/api/admin/inventory', {
        ...
        body: JSON.stringify({
          standOnlyStock: Number(data.get('standOnlyStock')),
          withCameraStock: Number(data.get('withCameraStock')),
        }),
      })
      ...
      setMessage(`库存已保存：带摄像头 ${result.withCamera.stock} 个，不带摄像头 ${result.standOnly.stock} 个。`)
  ...
  const products = [
    { key: 'withCameraStock', product: withCamera },
    { key: 'standOnlyStock', product: standOnly },
  ] as const
        ...
        {products.map(({ key, product }, index) => (
          <div key={key} className={`flex flex-col gap-4 p-5 sm:flex-row sm:items-center sm:justify-between ${index ? 'border-t border-white/10 md:border-l md:border-t-0' : ''}`}>
            ...
            <label className="flex items-center gap-3 text-xs font-black text-white/60">
              当前可售库存
              <input type="number" name={key} min="0" max="10000" step="1" required defaultValue={product.stock} className={inputClass} />
            </label>
          </div>
        ))}
      </div>
