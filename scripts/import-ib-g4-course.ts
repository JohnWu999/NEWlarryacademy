...
const dataPath = path.join(process.cwd(), 'data/ib-pyp-g4-course.json')
const vodMapPath = path.join(process.cwd(), 'data/ib-pyp-g4-vod-map.json')
...
async function main() {
  ...
  for (const lesson of course.lessons) {
    ...
    const activityId = `activity-ib-pyp-g4-ep${String(lesson.order).padStart(2, '0')}-practice`

    await prisma.lessonActivity.upsert({
      where: { id: activityId },
      update: {
        title: `${lesson.title} Practice Quest`,
        description: '10-question interactive quest with skill drills, points, gems, hints, sound feedback, and review advice.',
  ...
  console.log(`Imported ${course.lessons.length} IB G4 lessons into ${course.id}`)
}
