...
SOURCE_ROOT = pathlib.Path(
    os.environ.get(
        "IB_MYP_G6_UPLOAD_SOURCE_ROOT",
        "/Users/johnwu/Documents/自动视频剪辑项目/output/IB_MYP_G6_Math",
    )
)
COURSE_DATA = PROJECT_ROOT / "data" / "ib-myp-g6-course.json"
VOD_MAP = PROJECT_ROOT / "data" / "ib-myp-g6-vod-map.json"
...
def video_path_for(lesson):
    episode = f"{int(lesson['order']):02d}"
    episode_dir = SOURCE_ROOT / f"ib_myp_g6_ep{episode}"
    if not episode_dir.exists():
        raise RuntimeError(f"Missing episode folder: {episode_dir}")

    exact = episode_dir / lesson.get("videoFileName", "")
    if exact.exists():
        return exact

    mp4s = sorted(episode_dir.glob("*.mp4"), key=lambda item: item.stat().st_size, reverse=True)
    if not mp4s:
        raise RuntimeError(f"Missing .mp4 video in: {episode_dir}")
    return mp4s[0]
