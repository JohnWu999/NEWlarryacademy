...
const dataPath = path.join(process.cwd(), 'data/ngss-g6-science-course.json')
const vodMapPath = path.join(process.cwd(), 'data/ngss-g6-vod-map.json')
...
async function main() {
  ...
  for (const lesson of course.lessons) {
    ...
    const activityId = `activity-ngss-g6-${String(lesson.order).padStart(2, '0')}-practice`
    await prisma.lessonActivity.upsert({
      where: { id: activityId },
      update: {
        title: `${lesson.title} Practice Quest`,
        description: 'Interactive NGSS science quest with evidence, variables, models, and review guidance.',
  ...
  console.log(`Imported ${course.lessons.length} NGSS G6 lessons into ${course.id}`)
}
